...
class Player:
    ...
    """
    def __init__(self, screen, init_loc, init_items=[]):
        self.screen = screen
        self.loc = init_loc
        self.prev_loc = None
    ...
    def move(self, dir):
        ...
        if hasattr(self.loc, f"{dir}_to"):
            ...
            self.loc = dest[0]
            self.screen.print(f"{dest[1]}\n")
            return True
        else:
            self.screen.print(text_style['error']("ERROR: MOVEMENT NOT ALLOWED\n"))
            return False
        ...
        if item in self.loc.items:
            ...
            if result:
                if self.load + item.weight > self.max_load:
                    self.screen.print(f"Your pack is too full for the {item.name}.\n")
                    return False
                else:
                    self.items.append(item)
                    self.load += item.weight
                    self.screen.print(f"You pick up the {item.name}.\n")
                    item.on_pick_up()
                    return True
            else:
                return False
        else:
            self.screen.print("There's nothing here by that name.\n")
            return False

    def drop_item(self, item, quiet=False):
        if item in self.items:
            if self.loc.no_drop:
                self.screen.print("You don't think that's a good idea here.\n")
                return False
            self.items.remove(item)
            if quiet == False:
                self.screen.print(f"You set down the {item.name}.\n")
            self.load -= item.weight
            self.loc.add_item(item)
            return True
        else:
            self.screen.print("You don't have one of those in your inventory\n")
            return False
    ...
    def attack_mob(self, weapon, target):
        ...
        self.screen.print(random.choice(weapon.attack_text) + "\n")
        attack_chance = self.accuracy * weapon.accuracy
        dodge_chance = target.evasion
        if random.random() < attack_chance * (1 - dodge_chance):
            self.screen.print(f"{random.choice(target.text['dodge_fail'])}\n")
        else:
            target.health -= (self.strength / 10 ) * weapon.damage
            if target.health > 0:
                self.screen.print(f"{random.choice(target.text['dodge_success'])}\n")
            else:
                self.screen.print(random.choice(target.text["dead"]) + "\n")
                target.kill()
